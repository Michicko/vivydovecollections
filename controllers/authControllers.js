...
exports.login = catchAsync(async (req, res, next) => {
	...
	// if everything is okay send jwt to client
	user.password = undefined;
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
	...
	// generate random signToken
	const resetToken = user.createPasswordResetToken();
	await user.save({ validateBeforeSave: false });
	// send it back as email
	const resetUrl = `${req.protocol}://${req.get('host')}/api/v1/users/resetpassword/${resetToken}`;

	const message = `Forgot password? click the link to reset your password ${resetUrl}. \nIf you didn't request this link please ignore the message.`;
